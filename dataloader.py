...
def load_toy_data(
    ...
    (
        ...
        ids_train,
        ids_test,
    ) = train_test_split(C, X, labels, ids, test_size=0.2)
...
def load_data(
    ...
    test=DEFAULT_DATA_STATE["test"],
    disease_test=DEFAULT_DATA_STATE["disease_test"],
    dry_run=DEFAULT_DATA_STATE["dry_run"],
):
    # kwarg linting
    if disease_test is not None:
        disease_labels = False
        test = True

    # Old dry run code, generates random data. Now replaced by a data split before return, but commented because it might still be useful
    ...
    if covar_projection > 0:
        covar_pca = PCA(n_components=covar_projection, random_state=1).fit(
            C_train[:, transform_i:]
        )
    ...
    if feature_selection == "disease":
        ...
        for disease in np.unique(labels_train):
            ...
            var_avg += disease_vars * (
                np.sum(disease_idx) / len(disease_idx)
            )  # weight by tissue samples
...
if __name__ == "__main__":
    ...
    # load_data(disease_test='can I do this?')
