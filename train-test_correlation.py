...
from baselines import CorrelationNetwork, GroupedNetworks
...
# %%
savedir = f'saved_models/testcorrelation'
...
for boot_i in range(n_bootstraps):
    ...
    population_model = CorrelationNetwork().fit(X_boot)
    ...
    clustered_model = GroupedNetworks(CorrelationNetwork).fit(X_boot, cluster_labels_train[boot_idx])
    ...
    oracle_model = GroupedNetworks(CorrelationNetwork).fit(X_boot, labels_train[boot_idx])
...
for boot_i in range(n_bootstraps):
    ...
    contextualized_model = ContextualizedCorrelationNetworks(**model_kwargs)
...
for experiment, models in experiments.items():
    ...
    print(
        f"{experiment} Train MSE: {np.round(np.mean(train_mses[experiment]), 3)} +/- {np.std(np.mean(train_mses[experiment], axis=1))}")
    print(
...
w_train = [correlator.predict_correlation(C_train, squared=False, individual_preds=True)[0] for correlator in
           all_contextualized]
w_train = np.mean(w_train, axis=0)
w_test = [correlator.predict_correlation(C_test, squared=False, individual_preds=True)[0] for correlator in
          all_contextualized]
w_test = np.mean(w_test, axis=0)
np.save(f'{savedir}/correlation_networks_train.npy', w_train)
np.save(f'{savedir}/correlation_networks_test.npy', w_test)
# %%
import umap
import matplotlib as mpl
...
plt.legend()
plt.savefig(f'{savedir}/correlation_networks_embedding.pdf')
